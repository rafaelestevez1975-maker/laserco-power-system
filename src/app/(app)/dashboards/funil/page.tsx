...
export default async function DashFunilPage({ searchParams }: { searchParams: Promise<SP> }) {
  ...
  // ── Breakdown por dimensão (Unidade / Colaborador) — legado _dimCtrl/_breakdown L4510 ──
  const dim = sp.unidade ? 'colaborador' : 'unidade'
  // Conversão do funil por unidade: contagem real de concluídos por unidade (top 12).
  ...
  return (
    <div className="view active">
      ...
      {/* Apresentar por: Unidade / Colaborador (+ breakdown real) */}
      <div className="rel-card" style={{ marginBottom: 14 }}>
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <span className="os-st os-aberta">{dim === 'unidade' ? 'Unidade' : 'Colaborador'}</span>
      ...
      <div className="dash-w" style={{ margin: '0 0 12px' }}>
        <h4><i className="ti ti-filter-cog" /> {breakdownTitle}</h4>
        <BarChart title="" icon="ti-filter-cog" rows={breakdownChart} gold emptyMsg="Sem agendamentos no período." />
      </div>
        ...
        <div className="cli-scroll">
          <table className="cli-table">
            <thead>
              <tr>
                <th>#</th>
                <th>{dim === 'unidade' ? 'Unidade' : 'Colaborador'}</th>
