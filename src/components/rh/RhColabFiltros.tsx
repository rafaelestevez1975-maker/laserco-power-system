...
/**
 * Filtros da tela RH · Colaboradores. Espelha ColaboradoresFiltros, mas navega para
 * /rh/colaboradores (o componente original é fixo em /colaboradores e é compartilhado).
 */
export function RhColabFiltros({ areas, cargos }: { areas: string[]; cargos: [string, string][] }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center', margin: '4px 0 14px' }}>
      ...
      <select value={sp.get('cargo') ?? ''} onChange={(e) => setParams({ cargo: e.target.value })} style={selSt}>
        ...
        {cargos.map(([v, t]) => <option key={v} value={v}>{t}</option>)}
      </select>
      {areas.length > 0 && (
      ...
      {temFiltro && (
        <button className="btn" onClick={() => router.push('/rh/colaboradores')}><i className="ti ti-x" /> Limpar</button>
      )}
    </div>
  )
}
