...
export function ChamadosTabela({ tickets, atendentes, motivos, uniNome, unidades, todasTags = [] }: {
  ...
  return (
    <>
      {/* Tabela 1:1 com o legado (sacChamRows + relTable): colunas Protocolo, Cliente, Canal,
          Unidade, Motivo, Prioridade, Fase, Status e a coluna de ação (lápis). */}
...
function EditModal({ t, atendentes, motivos, unidades, todasTags, onClose, onSaved }: { t: ChamadoRow; atendentes: Atend[]; motivos: string[]; unidades: Unidade[]; todasTags: TagOpt[]; onClose: () => void; onSaved: () => void }) {
  ...
  // Modal 1:1 com o legado sacForm (index.html:9234): largura 840, grid de 3 colunas com a
  // mesma ordem de campos e rótulos, checkboxes em 2 colunas, observações e ficha do cliente.
      ...
      <div className="modal" style={{ width: 'min(1040px, 96vw)', maxWidth: 'min(1040px, 96vw)' }}>
        ...
        <div className="modal-body" style={{ display: 'block' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
            <div style={col}><label style={flab}>Nome do cliente *</label><input style={fin} value={f.nome_cliente} onChange={(e) => set('nome_cliente', e.target.value)} /></div>
            ...
            <div style={col}><label style={flab}>Canal *</label>
              <select style={fin} value={f.canal} onChange={(e) => set('canal', e.target.value)}>{canalOpts.map((c) => <option key={c} value={c}>{c}</option>)}</select>
            </div>
            ...
            <div style={col}><label style={flab}>Prioridade</label>
              <select style={fin} value={f.prioridade} onChange={(e) => set('prioridade', e.target.value)}>{PRIORIDADES.map((p) => <option key={p.k} value={p.k}>{p.l}</option>)}</select>
            </div>
            ...
            <div style={col}><label style={flab}>Fase (Kanban)</label>
              <select style={fin} value={f.fase} onChange={(e) => set('fase', e.target.value)}>{FASES.map((x) => <option key={x} value={x}>{x}</option>)}</select>
            </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginTop: 10 }}>
            <label style={{ fontSize: 12, display: 'flex', alignItems: 'center', gap: 6, cursor: 'pointer' }}><input type="checkbox" checked={f.multa_aplicada} onChange={(e) => set('multa_aplicada', e.target.checked)} /> Multa aplicada</label>
          ...
          <div style={{ marginTop: 14, borderTop: '1px solid var(--line)', paddingTop: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 8 }}>
              ...
              <button type="button" className="btn btn-ghost" style={{ fontSize: 12 }} disabled={fichaBusy} onClick={buscarFicha}><i className="ti ti-search" /> {fichaBusy ? 'Buscando…' : 'Buscar contratos, pagamentos e sessões'}</button>
            </div>
            {ficha && !ficha.achou && <div style={{ fontSize: 12.5, color: 'var(--text-3)', marginTop: 8 }}>Cliente não encontrado pelo CPF/telefone informado (ou sem permissão para ver a ficha).</div>}
            {ficha?.achou && (
        ...
        <div className="modal-foot"><button className="btn btn-ghost" onClick={onClose}>Cancelar</button><button className="btn btn-primary" disabled={busy} onClick={salvar}><i className="ti ti-check" /> {busy ? 'Salvando…' : 'Salvar chamado'}</button></div>
      </div>
    </div>
  )
}
