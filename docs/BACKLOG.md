...
- [x] ✅ **Service key do Supabase do site (`riut`)** — obtida via token de gerência; `SITE_SUPABASE_SERVICE_KEY` no `.env.local`. Ponte `/leads-site` conectada (**77 leads reais** lidos em 23/06). *(Rotacionar o `sbp_` que passou pelo chat.)*
- [ ] **Auto-match de unidade:** o site manda `unidade` como rótulo texto (ex.: "Florianópolis Centro — Florianópolis/SC") → casar com `unidades.id` para roteamento automático (hoje é manual no inbox).
