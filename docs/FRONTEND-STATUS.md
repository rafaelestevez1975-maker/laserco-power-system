...
| **Financeiro `/financeiro` (Contas a Pagar + Receber)**  **abas Pagar/Receber** com KPIs por RPC `fin_resumo` (somas precisas, migration 046) + **paginação** (há ~13k receitas); **"Dar baixa"** (pagar) conclui o chamado do SAC se for reembolso (`origem_ref_id`→ticket); **"Registrar recebimento"** (receber) marca o recebível como pago | ✅ Pagar+Receber (EPIC 9/15)  falta fluxo de caixa, DRE, régua de cobrança |
...
- ✅ **WhatsApp de disponibilidade** (avisarDisponibilidade pelo canal conectado) + **nota de triagem** (score_triagem_ia) no NotasModal.
- ⏳ **Falta:** envio real depende de canal conectado; **import SULTS/planilha** (EPIC 20.5);
