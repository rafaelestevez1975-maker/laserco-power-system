...
| **Ponte dos leads do site `/leads-site`** — caixa de entrada **dual-source**: lê a fonte REAL `riut.lasercompany_leads` quando há `SITE_SUPABASE_SERVICE_KEY` (senão fallback `lkii.site_leads` de teste, com aviso) e **roteia por unidade** (sac→`sac_tickets`, demais→`crm_leads`), marcando a origem como roteada | 🟡 conector pronto — **bloqueado pela service key do `riut`** (a anon não lê por RLS); falta auto-match de unidade |
